% Hechos: Síntomas de la esclerosis multiple 
sintoma('Espasmos'). 
...
prueba(puncion_lumbar, positivo). % indica bandas oligoclonales, que pueden indicar la presencia de la esclerosis multiple
prueba(electromiografia, negativo). % evalúa la actividad eléctrica de los musculos y los nervios que los controlan.
...
% Regla para verificar si un paciente tiene esclerosis multiple 
...
% Regla para clasificar el tipo de esclerosis multiple 
